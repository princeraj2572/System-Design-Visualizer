...
export default function NodePalette({ onDragStart }: NodePaletteProps) {
  ...
  return (
    <aside className="w-56 flex-shrink-0 bg-white dark:bg-slate-950 border-r border-slate-200 dark:border-slate-800 flex flex-col overflow-hidden">
      {/* Header */}
      <div className="px-3 pt-3 pb-2 border-b border-slate-100 dark:border-slate-800">
        <h2 className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">
          Components
        </h2>
        {/* Search */}
        <div className="relative">
          <Search size={11} className="absolute left-2 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-600 pointer-events-none"/>
          <input
            type="text"
            value={query}
      ...
      <div className="flex-1 overflow-y-auto px-2 py-2 space-y-3">
        {filtered ? (
          filtered.length === 0 ? (
            <p className="text-[10px] text-slate-400 dark:text-slate-600 text-center py-4">No results</p>
          ) : (
          ...
          NODE_CATEGORIES.map((category) => (
            <div key={category.id}>
              <div className="flex items-center gap-2 px-1 mb-1">
                <p className="text-[9px] font-bold text-slate-400 dark:text-slate-600 uppercase tracking-widest whitespace-nowrap">
                  {category.label}
                </p>
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <p className="text-[11px] font-semibold truncate text-slate-600 dark:text-slate-400
          group-hover:text-slate-900 dark:group-hover:text-slate-100 transition-colors">
          {config.label}
        </p>
        <p className="text-[9px] truncate text-slate-400 dark:text-slate-600 transition-colors">
